...
class OfferListTest {
    ...
    @Test
    void capacityIs81() {
        assertEquals(81, OfferList.MAX_OFFERS);
    }
    ...
    void addReturnsAssignedIndex() {
        ...
        assertEquals(2, list.add(apple(3, 300)));
    }
    ...
    @Test
    void getReturnsOfferAtIndex() {
        list.add(apple(5, 250));
        ShopOffer fetched = list.get(0);
        assertNotNull(fetched);
        assertEquals(250, fetched.priceBronze());
        assertEquals(5, fetched.quantityPerPurchase());
    }
    ...
    @Test
    void replaceNullReturnsFalse() {
        list.add(apple(1, 100));
        assertFalse(list.replace(0, null));
    }

    @Test
    void replaceOutOfBoundsReturnsFalse() {
        assertFalse(list.replace(0, apple(1, 100)));
        assertFalse(list.replace(-1, apple(1, 100)));
    }
    ...
    @Test
    void fromTagWithNoOffersTagReturnsEmpty() {
        CompoundTag empty = new CompoundTag();
        OfferList restored = OfferList.fromTag(empty);
        assertEquals(0, restored.size());
    }
}
